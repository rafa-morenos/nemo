...
- ✅ `Badge` (o Tag/Chip unificado) estendido com `size` (sm/md), `shape` (pill/square), `type="solid"` e `count` — fecha a spec proposta em `componentes-comuns-daki-ds.html` pra substituir os ~15 tags nativos dos produtos. Segundo componente (depois do Kanban) a chegar nas 3 plataformas: espelhado em RN (`Badge.tsx`) e Flutter (`NemoBadge` em `nemo_badge.dart`). Ressalva: `type="solid"` só é visualmente distinto do `filled` pra `default`/`inverted`/`disabled` — pra `success`/`warning`/`critical`/`info` renderiza igual ao `filled` de propósito, porque falta o token (ver backlog item 6).
- ✅ `Toast` (`sonner.tsx`) tematizado: os variants nativos do sonner (`success`/`error`/`warning`/`info`/`loading`) usam o mesmo par "soft" que `Badge`'s `type="filled"` (`surface-semantic-*`/`text-semantic-*` + borda semântica) em vez do `bg-popover` genérico; `loading` reusa `muted` por não ser uma cor semântica. Gotcha: as classes de cor de cada variant precisam de `!` (important) — sonner aplica o `classNames.toast` base e o `classNames[type]` no mesmo elemento, mesma especificidade, e a ordem de declaração das cores no `tailwind.preset.js` fazia `bg-popover` vencer o empate silenciosamente. Verificado em Storybook (light + dark, cores computadas via devtools).
...
4. **Migração dos toasts legados dos 5 produtos pro `Toast` (sonner.tsx) tematizado** (trabalho nos repos de cada produto, não no Nemo) — call-sites e esforço por stack já mapeados em
   `Produtos daki • Projetos front/toast-dev-spec-migracao.html` (seção "Call-sites a migrar"): HUBR (121 usos/7 módulos), Raio Web (14), Daki App (~40, maior esforço — duas famílias coexistindo), Raio App/Jake (13, sem `success`), Daki Web (1, trivial). Decisões de design ainda em aberto (truncate, fila vs. empilhar, swipe, dark mode no Jake) em `toast-design-guide-cenarios.html`.
