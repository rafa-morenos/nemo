...
- **Widgetbook, abordagem manual** (`WidgetbookComponent`/`WidgetbookUseCase` direto em `main.dart`/`lib/stories/`), não a abordagem geradora (`@widgetbook.UseCase` + `build_runner`) — evita depender de codegen pra algo deste tamanho.
...
- **`main.dart` seta `initialRoute`** pra abrir direto numa story em vez da tela de boas-vindas padrão do Widgetbook — o slug é `component/use-case` em lowercase com espaço virando hífen (espelha `WidgetbookNode.path`).
